...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, ori_img):
        ...
        if self.use_reid:
            # get feature from bbox image.
            features = self._get_features(bbox_xywh, ori_img)
            detections = [Detection(bbox_tlwh[i], conf, features[i]) for i, conf in enumerate(confidences) if
                          conf > self.min_confidence]
        else:
            detections = [Detection(bbox_tlwh[i], conf, None) for i, conf in enumerate(confidences) if
                          conf > self.min_confidence]
        ...
        for track in self.tracker.tracks:
            ...
            outputs.append(np.array([x1, y1, x2, y2, track_id,track.get_position_history()], dtype=object))
        if len(outputs) > 0:
